...
.hero{position:relative;overflow:hidden;padding:25px 30px;margin-bottom:18px;border-radius:24px;background:linear-gradient(135deg,rgba(255,255,255,.96),rgba(229,247,253,.94));border:1px solid rgba(0,140,190,.16);box-shadow:0 14px 38px rgba(20,85,115,.12)}
.hero:after{content:"";position:absolute;width:230px;height:230px;right:-70px;top:-105px;border-radius:50%;background:rgba(0,188,212,.12);animation:floatGlow 5s ease-in-out infinite}
.hero-title{font-size:2.25rem;font-weight:850;letter-spacing:.5px;margin:0;background:linear-gradient(90deg,#006fa6,#00a6c7,#075c96);-webkit-background-clip:text;-webkit-text-fill-color:transparent}
.hero-sub{color:#587381;margin-top:6px;font-size:.98rem}
@keyframes floatGlow{0%,100%{transform:translate(0,0)}50%{transform:translate(-10px,12px)}}
...
st.markdown("""<div class="hero"><div class="hero-title">💧 AMRIT AGASTYA</div><div class="hero-sub">Smart Water Purification • Quality Monitoring • Real-Time Intelligence</div></div>""", unsafe_allow_html=True)
